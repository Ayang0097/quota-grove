...
![Quota Grove Astral Terrarium quota themes](docs/screenshots/quota-grove-astral-terrarium-themes.png)

这套背景有独立的环境动效：森林为上升孢子、秋林为金色星尘、末日为逆风火屑、废土为六向霜晶。卡片每 3 秒轻量播放一次；双击则按 4、9、16、9、4 颗的五段密度触发 42 颗主题粒子，形成先少、后多、再少的完整动画。

![Quota Grove Astral Terrarium particle animation](docs/screenshots/quota-grove-astral-terrarium-animation.gif)

### 云海灯塔背景套系（macOS）

第三套“云海灯塔”采用更开阔的高空世界：同一座悬浮灯塔会从蓝色云海和青色信标，逐渐进入铜金黄昏、红紫雷暴，最终在银白云层中完全冻结。右键卡片打开“背景套系”即可切换，边框和进度条仍然跟随额度阶段变化。

![Quota Grove Cloudsea Beacon quota themes](docs/screenshots/quota-grove-cloudsea-beacon-themes.png)
...
![Quota Grove Cloudsea Beacon particle animation](docs/screenshots/quota-grove-cloudsea-beacon-animation.gif)

### 月光花房背景套系（macOS）

第四套“月光花房”采用克制的女性化新艺术风格：同一座月夜玻璃花房会从珍珠薰衣草与雾粉牡丹，逐渐进入玫瑰金暮色、石榴红蚀月和银灰冻结。画面保留左右文字区与底部进度条区域，边框和进度条仍跟随额度阶段同步变化。

![Quota Grove Moonlit Conservatory quota themes](docs/screenshots/quota-grove-moonlit-conservatory-themes.png)
...
![Quota Grove Moonlit Conservatory butterfly animation](docs/screenshots/quota-grove-moonlit-conservatory-animation.gif)

### 深海幻境背景套系（macOS）

第五套“深海幻境”进入一片无边框的生物荧光海域：同一片珊瑚海床会从青蓝繁盛，逐渐转为铜金衰退、石榴红海沟和银灰白化。画面完整铺满卡片，不包含额外观测窗框；边框和进度条继续随额度阶段同步变化。

![Quota Grove Abyssal Reverie quota themes](docs/screenshots/quota-grove-abyssal-reverie-themes.png)
...
![Quota Grove Abyssal Reverie jellyfish animation](docs/screenshots/quota-grove-abyssal-reverie-animation.gif)
...
![Quota Grove weather-linked rain animation](docs/screenshots/quota-grove-weather-rain-v130.gif)

![Quota Grove weather-linked dendritic snow animation](docs/screenshots/quota-grove-weather-snow-v130.gif)
