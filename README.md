...
macOS 卡片会在额度下降时播放落叶，并在卡片可见且系统未开启“减少动态效果”时，每 3 秒自动播放一次轻量环境动效。macOS 与 Windows 双击卡片都会在刷新额度的同时触发一次 48 片大量落叶：叶片统一从右上方倾斜进入，按 4、10、19、10、5 片的五段密度先少后多再少，向左的阵风推力逐渐衰减后由重力接管，横向空气阻力使轨迹自然收敛。叶片颜色跟随当前额度主题：森林为鲜绿、秋林为橙红、末日为暗红褐、废土为灰白。
...
`--snapshot-json` 只输出标准化额度快照。`--self-test` 覆盖 47 项主题边界、视觉规则、落叶粒子密度、阵风、重力与景深、解析和中英文切换检查，包括 PRD 指定的 50、49、20、19、3、2、1、0 八个主题边界。
